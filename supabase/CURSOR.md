...
|----------|------|-------|--------|---------|
| `parse-intent` | `functions/parse-intent/index.ts` | `{ transcript: string }` | Dials + filters; **429** `{ error, code: "rate_limit", retry_after_ms }`; ROE-003 strips carrier-only dish + celebratory dial band; **IP-FIX** transcript lens + no Healthy cuisine + negation dietary (`_shared/intent-sanitize.ts`) | `GEMINI_API_KEY` |
...
| `functions/_shared/intent-sanitize.ts` | IP-FIX transcript grounding (cuisine / dietary / lens / sweet) — sync with `src/lib/intentSanitize.ts` |

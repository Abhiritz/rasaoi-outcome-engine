...
|----------|------|-------|--------|---------|
| `parse-intent` | `functions/parse-intent/index.ts` | `{ transcript: string }` | Dials + filters; **429** `{ error, code: "rate_limit", retry_after_ms }`; ROE-003 strips carrier-only dish + celebratory dial band | `GEMINI_API_KEY` |
